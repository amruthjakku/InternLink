...
async function testTaskProgressSystem() {
  ...
  try {
    ...
    if (cohortInterns.length === 0) {
      console.log('❌ No interns found in the cohort. Please add interns first.');
      return;
    }
    
    console.log(`✅ Found ${cohortInterns.length} interns in cohort:`);
    cohortInterns.forEach(intern => {
    ...
    for (let i = 0; i < cohortInterns.length; i++) {
      const intern = cohortInterns[i];
    ...
    const leaderboardData = await TaskProgress.aggregate([
      {
        $match: { 
          internId: { $in: cohortInterns.map(i => i._id) },
    ...
    if (cohortTask.subtasks && cohortTask.subtasks.length > 0) {
      ...
      const testIntern = cohortInterns[0];
      let progress = await TaskProgress.findOne({
        taskId: cohortTask._id,
      ...
      await progress.save();
      console.log(`   ✅ Updated subtask progress for ${testIntern.name}`);
    ...
    console.log(`   ✅ Interns tested: ${cohortInterns.length}`);
